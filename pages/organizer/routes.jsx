...
import { AICopilotEventAssistant } from '@/components/ai/AICopilotEventAssistant'
import { AIFeedbackInsights } from '@/components/ai/AIFeedbackInsights'
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        ...
        <form className="grid gap-4" onSubmit={handleSubmit}>
          ...
          <AICopilotEventAssistant form={form} />
...
function OrganizerFeedbackPage() {
  ...
  const primaryEventId = feedbacks?.[0]?.eventId || feedbacks?.[0]?.event?._id || feedbacks?.[0]?.event?.id
  return (
    <div className="space-y-6">
      <PageHeader eyebrow="Organizer" title="Student feedback" description="Ratings and comments for your events." />
      {primaryEventId ? <AIFeedbackInsights eventId={primaryEventId} /> : null}
      {feedbacks.length ? (
